...
class cave_dataset(data.Dataset):
    def __init__(self, opt, HR_HSI, HR_MSI,file_list,istrain = True):
        super(cave_dataset, self).__init__()
        self.istrain  =  istrain
        self.factor = opt.scale_ratio
        self.file_list = file_list
        self.arch = opt.arch
        if istrain:
            self.num = opt.trainset_num
            self.file_num = 20
            self.sizeI = opt.patch_size
        else:
    ...
    def __getitem__(self, index):
        ...
        img_name = self.file_list[index1]
        ...
        if self.istrain:
            px      = np.random.randint(0, 512-self.sizeI)
            py      = np.random.randint(0, 512-self.sizeI)
        else:
            px = 0
            py = 0
        ...
        return lr_hsi, hr_msi, hr_hsi,img_name
